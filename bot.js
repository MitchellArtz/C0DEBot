...
if (command === "eval3"){
    if (message.author.id !== config.ownerid2) return message.reply("You do not have permission to use this.")
var str = args.join(" ");
var patt = new RegExp("token");
var res = patt.test(str);
...
try {
  ...
  var code = args.join(" ");
  var evaled3 = eval(code);

  if (typeof evaled3 !== "string")
    evaled3 = require("util").inspect(evaled3);
  //message.channel.sendMessage(":inbox_tray: **INPUT**\n")
  message.channel.sendEmbed(new Discord.RichEmbed().addField("Javascript Eval:", "Success!").addField(":inbox_tray: **INPUT**", "```" + args.join(" ") + "```").addField(":outbox_tray: **OUTPUT**", "```" + clean(evaled3) + "```").setColor(message.guild.me.displayHexColor))
  //message.channel.sendCode("xl", args.join(" "));
  //message.channel.sendMessage(":outbox_tray: **OUTPUT**\n")
